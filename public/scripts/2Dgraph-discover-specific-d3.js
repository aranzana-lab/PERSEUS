////Pedigree search of specific individuals
var indiv1 = document.getElementById("searched-name").innerHTML;
var jumps1 = document.getElementById("searched-number").innerHTML;

console.log(indiv1, jumps1);

//Literature used
papers(speciesSel);

axios //Axios connection to the main search on Neo4j
  .post("/perseus/api/specific", {
    species: `${speciesSel}`,
    individual: `${indiv1}`,
    jumps: jumps1,
  })
  .then(function (response) {
    ...
    console.log(result);
    function createRel(relationship) {
      relationship.value = relationship.value;
    }
    obtainResultsNeo4j(result, createRel); //Obtain links from Neo4j results

    // session.close(); //Close session
    consoleLength(links, start); //Console the number of links ambd how much time it needs to load.
    specificLinks(links); //Update links with unique specific relationships
    generateGData(nodes, links2); //General Data
    ...
    function ForceGraph(
      ...
      customizeBar = document.getElementById("customize-pedigree-search");
